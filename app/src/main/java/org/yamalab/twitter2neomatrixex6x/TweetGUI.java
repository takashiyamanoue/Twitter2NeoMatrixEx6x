...
package org.yamalab.twitter2neomatrixex6x;
...
import org.yamalab.twitter2neomatrixex6x.R;
...
implements OnClickListener, OnCheckedChangeListener
{
	...
	private Button mSetDisplayButton;
...
//        setContentView(R.layout.tweet);
    	...
    	mSetDisplayButton=(Button) findViewById(R.id.setDisplayButton);
    	mSetDisplayButton.setOnClickListener(this);
    	mSetDefaultButton=(Button) findViewById(R.id.setDefaultButton);
...
//            hashTag = sb.toString();                        
		...
		if(v==mSetDisplayButton){
//        	SpannableStringBuilder sb = (SpannableStringBuilder)edit.getText();
//            hashTag = sb.toString();
			String toAdk=mResultField.getText().toString();
			activity.sendCommandToService("adk command: set message "+toAdk,"");
//			activity.sendCommandToService("twitter set downloadHashTag",(mDownloadHashTagField.getText()).toString());
		}
		else
		if(v==mSetDefaultButton){
			setInitialDisplayText();
		}
    }
    private void setInitialDisplayText(){
		((Activity)mHostActivity).runOnUiThread(new Runnable() {
			@Override
			public void run(){
				mResultField.setText("(red)Please (green)Tweet (blue)Something (yellow)with (white)"+mDownloadHashTagField.getText().toString()+"!");
			}
		});
    	onClick(mSetDisplayButton);
	}
